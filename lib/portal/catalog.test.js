...
test('all nine products retain their assigned canonical domains and order', () => {
  ...
  const verifiedHosts = new Set(['cycletag.eu', 'getgatezero.com', 'viesproof.eu', 'pay.nyttolabs.com', 'failclosed.nyttolabs.com', 'webhook-witness.vercel.app', 'rfq-stamp.vercel.app', 'ragefile.vercel.app', 'ai-venture-worker-app.vercel.app'])
  for (const product of PORTAL_PRODUCTS) {
    assert.equal(new URL(product.url).protocol, 'https:')
    assert.ok(verifiedHosts.has(new URL(product.url).hostname))
  }
})
