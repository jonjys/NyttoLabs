import { MongoClient } from 'mongodb'
import { mongoConnectionString, isMongoConfigured, mongoDatabaseName } from './mongo-url'

// Reuse a single Mongo connection across hot reloads / invocations.
let client
let db

export { isMongoConfigured, mongoConnectionString }

export class MongoConfigError extends Error {
  constructor(message) {
    super(message)
    this.name = 'MongoConfigError'
    this.code = 'MONGO_NOT_CONFIGURED'
  }
}

export async function getDb() {
  const uri = mongoConnectionString()
  if (!uri) {
    throw new MongoConfigError('MONGO_URL (or MONGODB_URI) is missing or not a mongodb:// URI')
  }
  if (!db) {
    client = new MongoClient(uri)
    await client.connect()
    db = client.db(mongoDatabaseName())
  }
  return db
}

export const COLLECTIONS = {
  applications: 'applications',
  actions: 'actions',
  partners: 'partners',
  offers: 'offers',
  routingRules: 'routing_rules',
  clickEvents: 'click_events',
  conversions: 'conversions',
  experiments: 'experiments',
  experimentAssignments: 'experiment_assignments',
  partnerInquiries: 'partner_inquiries',
  contactInquiries: 'contact_inquiries',
  auditLogs: 'audit_logs',
  settings: 'settings',
  adminSessions: 'admin_sessions',
}

// Strip Mongo internal _id for safe JSON responses.
export function clean(doc) {
  if (!doc) return doc
  if (Array.isArray(doc)) return doc.map(clean)
  const { _id, ...rest } = doc
  return rest
}

export function isDemoMode() {
  return String(process.env.DEMO_MODE || '').toLowerCase() === 'true'
}

// Demo mode can be overridden from the dashboard (settings.demoMode). Falls back to env.
export async function getDemoMode(db) {
  try {
    if (!isMongoConfigured() && !db) return isDemoMode()
    const database = db || (await getDb())
    const s = await database.collection(COLLECTIONS.settings).findOne({ key: 'global' })
    if (s && typeof s.demoMode === 'boolean') return s.demoMode
  } catch (e) {
    console.error('getDemoMode error', e)
  }
  return isDemoMode()
}

export async function audit(actor, action, entity, entityId, meta = {}) {
  try {
    if (!isMongoConfigured()) return
    const database = await getDb()
    await database.collection(COLLECTIONS.auditLogs).insertOne({
      id: crypto.randomUUID(),
      actor: actor || 'system',
      action,
      entity,
      entity_id: entityId || null,
      meta,
      created_at: new Date(),
    })
  } catch (e) {
    // Never let auditing break the main flow.
    console.error('audit error', e)
  }
}
