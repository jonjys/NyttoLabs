...
import { seedApplications, PUBLIC_HELLO_EMAIL, PUBLIC_SUPPORT_EMAIL } from './catalog'
...
export function seedSettings() {
  ...
  return {
    ...
    partnerEmail: PUBLIC_HELLO_EMAIL,
    defaultCurrency: 'SEK',
...
// Idempotent seeding. Known portfolio apps are $set on every boot so stale
// Mongo URLs and public status (www vs apex, leftover Building, etc.) cannot linger.
export async function ensureSeed(db) {
  ...
  if (!existingSettings) {
    await db.collection(COLLECTIONS.settings).insertOne(seedSettings())
  } else if (String(existingSettings.partnerEmail || '').toLowerCase() === 'partners@nyttolabs.com') {
    await db.collection(COLLECTIONS.settings).updateOne(
      { key: 'global' },
      { $set: { partnerEmail: PUBLIC_HELLO_EMAIL, updated_at: new Date() } },
    )
  }
