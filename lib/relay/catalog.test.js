...
describe('canonical product catalog', () => {
  it('exposes CycleTag, Vatidence, GateZero and Failclosed with canonical URLs', () => {
    const products = defaultPublicProducts()
    const bySlug = Object.fromEntries(products.map((p) => [p.slug, p]))

    assert.equal(CANONICAL_PRODUCT_URLS.cycletag, 'https://cycletag.eu/')
    assert.equal(CANONICAL_PRODUCT_URLS.viesproof, 'https://vatidence.nyttolabs.com/')
    ...
    assert.equal(bySlug.viesproof.name, 'Vatidence')
    assert.equal(bySlug.viesproof.url, 'https://vatidence.nyttolabs.com/')
    ...
    assert.equal(bySlug.cycletag.section, 'live')
  ...
  it('overrides stale Mongo URLs with canonical hosts', () => {
    ...
    assert.equal(vies.name, 'Vatidence')
    assert.equal(vies.url, 'https://vatidence.nyttolabs.com/')
  ...
  it('keeps the public grid populated when Mongo is empty or fails', () => {
    for (const input of [null, undefined, [], [{ slug: 'nytto-relay', publicVisible: true }]]) {
      ...
      assert.equal(products.length, 5)
      assert.deepEqual(products.map((p) => p.slug).sort(), ['curl-to-buy', 'cycletag', 'failclosed', 'gatezero', 'viesproof'])
    }
  })
  ...
  it('never publishes private or internal apps on the public grid', () => {
    ...
    assert.equal(products.find((p) => p.slug === 'cycletag').status, 'live')
  ...
  it('does not let Mongo override canonical public status', () => {
    const products = mergePublicCatalog([
      { name: 'CycleTag', slug: 'cycletag', publicVisible: true, status: 'building', url: 'https://www.cycletag.eu/' },
      { name: 'Vatidence', slug: 'viesproof', publicVisible: true, status: 'building', url: 'https://vatidence.nyttolabs.com/' },
    ...
    assert.equal(products.find((p) => p.slug === 'cycletag').status, 'live')
    assert.equal(products.find((p) => p.slug === 'cycletag').section, 'live')
    assert.equal(products.find((p) => p.slug === 'cycletag').url, 'https://cycletag.eu/')
    assert.equal(products.find((p) => p.slug === 'viesproof').status, 'live')
    assert.equal(products.find((p) => p.slug === 'viesproof').url, 'https://vatidence.nyttolabs.com/')
...
describe('public status sections', () => {
  it('omits empty status sections, including Building when nothing is building', () => {
    const groups = groupPublicProductsByStatus(defaultPublicProducts())
    assert.deepEqual(groups.map((g) => g.key), ['live', 'public-beta', 'private-beta'])
    assert.equal(groups.some((g) => g.key === 'building'), false)
    assert.deepEqual(
      groups.find((g) => g.key === 'live').list.map((p) => p.slug),
      ['cycletag', 'viesproof', 'curl-to-buy'],
    )
    assert.deepEqual(
      groups.find((g) => g.key === 'public-beta').list.map((p) => p.slug),
      ['gatezero'],
    )
    assert.deepEqual(
      groups.find((g) => g.key === 'private-beta').list.map((p) => p.slug),
      ['failclosed'],
    )
  })

  it('includes Building only when a public building product exists', () => {
    const groups = groupPublicProductsByStatus([
      ...defaultPublicProducts(),
      { slug: 'wip', status: 'building', name: 'WIP' },
    ])
    const building = groups.find((g) => g.key === 'building')
    assert.equal(Boolean(building), true)
    assert.deepEqual(building.list.map((p) => p.slug), ['wip'])
    assert.equal(PUBLIC_STATUS_GROUPS.some((g) => g.key === 'building'), true)
  })

  it('treats missing or empty catalogs as no sections', () => {
    assert.deepEqual(groupPublicProductsByStatus(null), [])
    assert.deepEqual(groupPublicProductsByStatus([]), [])
  })
})
