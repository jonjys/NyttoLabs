...
export const PUBLIC_PORTFOLIO_SLUGS = ['cycletag', 'viesproof', 'gatezero', 'failclosed', 'curl-to-buy']

// Keep stable internal slugs while enforcing current public product branding.
export const PUBLIC_PRODUCT_NAMES = { viesproof: 'Vatidence' }
...
export const CANONICAL_PRODUCT_URLS = {
  ...
  viesproof: 'https://vatidence.nyttolabs.com/',
  gatezero: 'https://getgatezero.com/',
  failclosed: 'https://failclosed.nyttolabs.com/',
  // Deployed from its own repository (jonjys/curl-to-buy), not from this one.
  'curl-to-buy': 'https://pay.nyttolabs.com/',
}
...
export function seedApplications() {
  ...
  return [
    ...
    },
    {
      ...
      name: 'Curl-to-Buy',
      slug: 'curl-to-buy',
      description: 'Sell a digital file without an account on either side. Upload it, set a price, share the link. Buyers pay by card and get a time-limited download.',
      url: CANONICAL_PRODUCT_URLS['curl-to-buy'],
...
export function publicProductShape(app) {
  ...
  return {
    name: PUBLIC_PRODUCT_NAMES[slug] || app.name,
    slug,
