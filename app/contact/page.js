      ...
      <section className="mx-auto max-w-3xl px-5 py-16">
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2">
          <div className="rounded-xl border border-black/10 bg-white p-6">
            ...
            <a className="mt-1 block text-sm text-emerald-800 underline" href={`mailto:${s?.supportEmail || 'hello@nyttolabs.com'}`}>{s?.supportEmail || 'hello@nyttolabs.com'}</a>
          </div>
          <div className="rounded-xl border border-black/10 bg-white p-6">
            <Handshake className="h-5 w-5 text-emerald-800" />
            <h3 className="mt-4 font-semibold">Partnerships</h3>
            <a className="mt-1 block text-sm text-emerald-800 underline" href={`mailto:${s?.partnerEmail || 'partners@nyttolabs.com'}`}>{s?.partnerEmail || 'partners@nyttolabs.com'}</a>
