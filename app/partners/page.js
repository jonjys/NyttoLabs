...
export default function PartnersPage() {
  ...
  const partnerEmail = settings?.partnerEmail || 'partners@nyttolabs.com'
  const field = 'mt-1 w-full rounded-md border border-black/15 bg-white px-3 py-2 text-sm outline-none focus:border-emerald-700'
    ...
    <div className="min-h-screen bg-[#f7f5f0] text-[#1b1b16]">
      ...
      <section className="mx-auto max-w-6xl px-5 pt-16 pb-8">
        ...
        <h1 className="mt-5 max-w-3xl text-3xl font-semibold tracking-tight sm:text-4xl">
          Meet customers at the moment they need the next product, service, or action.
        </h1>
        <p className="mt-4 max-w-2xl text-[#4a4a3e]">
          Nytto Relay is the invisible routing layer underneath Nytto Labs products. When a real commercial
          intent appears, Relay connects that user to the most relevant, useful destination — yours.
        </p>
      </section>
        ...
        <div className="grid gap-10 lg:grid-cols-5">
          <div className="lg:col-span-2">
            ...
            <p className="mt-3 text-sm leading-relaxed text-[#4a4a3e]">
              Tell us about your products and markets. We’ll assess relevance to our users and follow up.
              Prefer email? Reach us at <a className="text-emerald-800 underline" href={`mailto:${partnerEmail}`}>{partnerEmail}</a>.
            </p>
          </div>
          <form onSubmit={submit} className="lg:col-span-3 rounded-xl border border-black/10 bg-white p-6">
            ...
            <button disabled={submitting} className="mt-5 inline-flex items-center gap-2 rounded-md bg-[#1b1b16] px-5 py-2.5 text-sm font-medium text-[#f7f5f0] transition hover:bg-black disabled:opacity-60">
